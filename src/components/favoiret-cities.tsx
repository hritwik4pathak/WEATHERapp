...
return(
    ...
    <div onClick={()=> navigate(`/cty/${name}?lat=${lat}&lon=${lon}`)}
    role="button"
    tabIndex={0}
    className="relative flex min-w-[250px] cursor-pointer items-center gap-3 rounded-lg borfer bg-card p-4 pr-8 shadow-sm transition-all hover:shado-md"
    >
    <Button 
    variant="ghost"
    size="icon"
    className=" absolute right-1 top-1 h-6 w-6 rounded-full p-0 
             hover:text-destructive-foureground group-hover:opacity-100"
    onClick={(e)=>{
        e.stopPropagation();
        onRemove(id);
